...
export function QualifierQueue({ show, cibles }: { show: Show; cibles: CibleEnrichie[] }) {
          ...
          <div key={c.id} className="card flex flex-wrap items-center gap-3 px-4 py-3">
            ...
            {ARCHETYPE_ORDER.map((a) => (
              <button
                key={a}
                disabled={occupe}
                onClick={() => agir(c.id, () => qualifierCible({ cible_id: c.id, archetype: a, priorite: prio, show_slug: show.slug }))}
