...
interface LigneJob { objectif: string; model: string | null; tokens_in: number | null; tokens_out: number | null }
...
async function jobsDepuis(sb: SB, depuisIso: string): Promise<LigneJob[] | null> {
  try {
    const { data, error } = await sb
      ...
      .limit(5000);
    if (error) return null; // colonnes absentes : migration 0039 non appliquée
    return (data ?? []) as LigneJob[];
  } catch {
    return null;
  }
}
...
function somme(jobs: LigneJob[]): number {
  return jobs.reduce((acc, j) => acc + coutEstimeEur(j.model, j.tokens_in ?? 0, j.tokens_out ?? 0), 0);
}
...
export async function ventilationMois(
  sb: SB,
  now = Date.now()
): Promise<{ objectif: string; jobs: number; tokens_in: number; tokens_out: number; cout_eur: number }[]> {
  const jobs = await jobsDepuis(sb, debutMoisIso(now));
  if (!jobs) return [];
  const par = new Map<string, { objectif: string; jobs: number; tokens_in: number; tokens_out: number; cout_eur: number }>();
  for (const j of jobs) {
    const cle = j.objectif.startsWith("fiche:") ? j.objectif : "profil";
    const cur = par.get(cle) ?? { objectif: cle, jobs: 0, tokens_in: 0, tokens_out: 0, cout_eur: 0 };
    cur.jobs += 1;
    cur.tokens_in += j.tokens_in ?? 0;
    cur.tokens_out += j.tokens_out ?? 0;
    cur.cout_eur += coutEstimeEur(j.model, j.tokens_in ?? 0, j.tokens_out ?? 0);
    par.set(cle, cur);
  }
