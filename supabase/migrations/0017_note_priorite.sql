-- Débrief 26/06 + priorisation : un champ note durable (contexte de fond, pour
-- personne ET entreprise, distinct du journal de touches) et une note de
-- priorité manuelle 1-5 (pour remonter 5-10 fiches en tête de colonne).

alter table public.cibles add column if not exists note text;
alter table public.cibles add column if not exists note_priorite smallint check (note_priorite between 1 and 5);

-- Recréer la vue pour exposer note / note_priorite (c.* figé à la création).
