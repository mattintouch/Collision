-- Archivage des cibles : sortir une fiche du board sans la détruire (ancien
-- invité, allié-seul…). Elle reste cherchable et reliable comme appui.

alter table public.cibles add column if not exists archive boolean not null default false;

-- Recréer la vue pour exposer `archive` (c.* est figé à la création).
